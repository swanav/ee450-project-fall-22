...
tcp_client_t* tcp_client_connect(tcp_endpoint_t* dest, tcp_receive_handler_t on_receive, tcp_disconnect_handler_t on_disconnect) {
    ...
    if (!client) {
        LOGEM("tcp_client_connect: Error allocating memory for client");
        return NULL;
    }
    ...
    if (client->sd < 0) {
        LOGEM("tcp_client_connect: Error creating socket");
        free(client);
        return NULL;
    }

    if (connect(client->sd, (struct sockaddr*)&dest->addr, dest->addr_len) < 0) {
        LOGEM("tcp_client_connect: Error connecting to server");
        free(client);
        return NULL;
    }
...
void tcp_client_receive(tcp_client_t* client) {
    if (client) {
        ...
        if (bytes_read > 0) {
            ...
            LOGI("Received %d bytes from server", bytes_read);
